...
#include "interception/UdpHandler.h"
...
namespace Proxirae {
	...
	bool UdpHandler::CanHandle(const IPacketContext& ctx)
	{
		return ctx.IsUdp();
	}

	void UdpHandler::Handle(PacketDispatchContext& ctx)
	{
		auto& packetCtx = ctx.packetCtx;

		if (packetCtx.IsOutbound()) {
			if (ntohs(packetCtx.GetSourcePort()) == m_redirectPort) {
				ThreeTuple key{
