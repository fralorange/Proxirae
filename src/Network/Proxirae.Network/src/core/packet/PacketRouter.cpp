...
namespace Proxirae {
	PacketRouter::PacketRouter(IProcessResolver& resolver, RuleEvaluator& evaluator, std::set<RuleContract>& rules, ConnectionRegistry& connections)
		: m_resolver(resolver), m_evaluator(evaluator), m_rules(rules), m_connections(connections) { }
